...
@ns_difficulty.route("/conpersec/res")
class Wikification2conpersecresDifficulty(Resource):
    '''This is a test'''
    @ns_difficulty.expect(input_def, validate=True)
    @ns_difficulty.marshal_with(output_def)
    def post(self):
        '''Compute 'ConceptPerSec' difficulty scores for a given resources in the DB'''
